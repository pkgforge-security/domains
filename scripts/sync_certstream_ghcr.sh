...
if [[ -n "${I_DATES[*]}" && "${#I_DATES[@]}" -ge 1 ]]; then
  ...
  #Download
   for I_D in "${I_DATES[@]}"; do 
    echo -e "\n[+] Processing ${I_D}"
     #Set
      unset GHCRPKG_TAG INPUT_TMP NO_GZ SRC_URL TXT_FILE
      ...
      for i in {1..2}; do
        ...
        #  fi
          ...
          if [[ -s "${TMPDIR}/${I_D}.txt.gz" && $(stat -c%s "${TMPDIR}/${I_D}.txt.gz") -gt 1000000 ]]; then
             ...
             #Extract
               7z x -y "${TMPDIR}/${I_D}.txt.gz"
               TXT_FILE="$(find "${TMPDIR}" -type f -exec file -i "{}" \; | grep -Ei "text/plain" | cut -d":" -f1 | xargs realpath | grep -Ei "${I_D}|latest" -m1 | tr -d '[:space:]')"
             #Copy
               if [[ ! -s "${TXT_FILE}" || $(stat -c%s "${TXT_FILE}") -lt 1000000 ]]; then
                ...
                break
                 ...
                 rm -rf "${ORAS_LOCAL}/DATA/certstream" 2>/dev/null
                 mkdir -p "${ORAS_LOCAL}/DATA/certstream"
                 cp -fv "${TXT_FILE}" "${ORAS_LOCAL}/DATA/certstream/${I_D}.txt"
